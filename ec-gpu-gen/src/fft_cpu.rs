    ...
    for _ in 0..log_n {
        ...
        while k < n {
            ...
            for j in 0..m {
                ...
                tmp.sub_assign(&t);//tmp -= t;
                a[(k + j + m) as usize] = tmp;
                a[(k + j) as usize].add_assign(&t);//a[(k + j) as usize] += t;
                w.mul_assign(&w_m);//w *= w_m;
            }
        ...
        for _ in 0..5 {
            for log_d in 0..10 {
                ...
                for log_threads in log_d..min(log_d + 1, 3) {
                    parallel_fft::<Bn256>(&mut v1_coeffs, &worker, &v1_omega, log_d, log_threads);
